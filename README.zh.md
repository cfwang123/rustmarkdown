...
- 三模式切换（代码、侧边预览、预览；侧栏可拖分隔条）；切换时保持屏幕顶部那一行仍在最上
...
- 输入界面 Esc 取消：Vim 密码 / 未保存关闭 / 退出 / 重载 / 错误 / 关于对话框，查找条，大纲筛选，资源管理器路径栏，参数设置
- 源码与预览：双击扩选到中英文标点之前（空格算进词里；`: ： % # / \` 与 `.` 不算分隔，URL/路径/锚点整段选中）；三击选中一行（预览为折行后的视觉行）；选区两端空白会去掉。源码双击选区会保持（不再闪一下消失）。复制时挤掉汉字与英文/数字之间多出来的空格。源码双击不再卡 0.5～1 秒（长文滚下去后也是）
...
- 检查更新（帮助菜单「检查更新...」）：从 GitHub Releases 查最新版，下载 `.7z` 后退出程序、自动解压覆盖安装目录并重启（对齐 ScreenKit `AppUpdater`）；启动时按设置间隔自动检查（默认 7 天、0 = 关闭；间隔与上次检查时间写入 `settings.json`），发现新版本弹窗询问；GitHub 走本机代理 `127.0.0.1:7897`，代理不可用时直连
- 无参数启动恢复上次关闭时打开的文件、视图模式与滚动位置（`session.json`），并恢复目录树工作区根目录和主窗口宽高/位置（若窗口几乎完全在屏幕外则位置改回系统默认、宽高仍用上次的）；多标签只真正打开当前标签，其它标签切过去时再读盘；之后再次打开同一文件也会恢复上次的模式与位置；命令行传入路径则按参数打开（传入的是文件时仍恢复上次工作区）
...
- Ctrl+F 查找：不区分大小写，F3 / Shift+F3 上下一个（查找框有焦点时也可用）；源码与预览高亮；Excel 按单元格搜索并跳转
...
- 窗口标题栏显示版本号（如 `demo.md — rustmarkdown v1.0.2`）
...
- 打包：Node.js、7-Zip（`7z` 在 PATH 中，或安装到默认目录）；程序自更新解压 `.7z` 也需要 7-Zip
