...
- 多标签：拖动排序、中键关闭、右键关闭其它/全部、Ctrl+Shift+T 重开、历史文件（最近 20）；拖出拆窗、拖入合并。
...
- **检查更新**（帮助菜单）：查 GitHub Releases、下载 `.7z`（带进度）后退出并自动覆盖安装目录、完成后重启（对齐 ScreenKit）；启动时按间隔自动检查（默认 7 天、0 关闭）；GitHub API 直连。
