...
- 源码与预览：双击扩选到空白、`' " ( )`、反引号、逗号、中文标点 `，。、‘“’”（）？` 之前的字符（`: / \` 算在词里）；三击选中一行；选区两端空白会去掉（只保留中间文字，点在空白上则不选）。源码双击选区会保持（不再闪一下消失）。预览双击选中当前段/列表项整段；复制时挤掉汉字与英文/数字之间多出来的空格，并 trim 两端空白。源码双击不再卡 0.5～1 秒（长文滚下去后也是）
