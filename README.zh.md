...
- Ctrl+Z / Ctrl+Y 撤销重做后光标留在改动处，视口不再跳到更早的编辑位置；撤销回打开/保存时的正文后标签变为未修改
