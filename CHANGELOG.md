...
- Help → **Check for Updates...**: queries GitHub Releases, downloads the `.7z` (progress, cancel), then quits, replaces the install directory and restarts — GitHub via local proxy `127.0.0.1:7897` with direct fallback; updater runs as `--apply-update` CLI (wait old process → extract `tmp/update_extract` → overwrite, skipping tmp/target/log/.git). Startup auto-check on the interval (default 7 days, 0 = off; `updateCheckDays` / `lastUpdateCheck` in settings.json, written only after a successful check).
...
- 帮助菜单「检查更新...」：查 GitHub Releases、下载 `.7z`（带进度、可取消）后退出并自动覆盖安装目录、完成后重启（对齐 ScreenKit `AppUpdater`）——GitHub 走本机代理 `127.0.0.1:7897`、失败直连；更新器为 `--apply-update` 命令行（等旧进程退出 → 解压 `tmp/update_extract` → 覆盖，跳过 tmp/target/log/.git）。启动时按间隔自动检查（默认 7 天、0 关闭；间隔与上次检查时间写 `settings.json`，成功检查后才更新时间）。
...
- XLS / XLSX / XLSM 只读预览（对齐 docview 虚拟网格）：表页签、行列表头冻结、拖选复制 TSV、Ctrl+滚轮缩放、PgUp/PgDn 换表、大纲为表名。
- PDF 预览不再空转卡顿（只画可见页、合并过时光栅、缩放拉伸旧图、视口外丢纹理，对照 SumatraPDF）；千页拖滚动条快速（页顶二分排可见页、拖条暂停光栅）；文本页双击不再当图片弹层（100% ⇄ 适宽）。
- DOC / DOCX 只读预览按 docview DocxViewer 重做：`office_oxide` 排版模型直接分页绘制（不再转 Markdown），保留标题/字号/加粗/颜色/对齐/列表编号/表格/图片，另存可导出 `.md`。
...
- 图片文件只读预览（对齐 docview ImageViewer）：居中适应、滚轮缩放、拖拽平移、双击适合/100%、`[` `]` 旋转、Ctrl+C/右键复制、另存 png/jpg/bmp。
...
- M2 移植 MdParser（块/行内/表格/details/img、源行映射）；预览原生渲染（标题编号、列表、任务勾选、引用、code syntect、表格列宽、图片、链接）；编辑 180ms 防抖重解析；`--selftest` 对照 docview 用例。
...
- Markdown 源码语法高亮（对齐 docview vim 风：标题分层色、标记灰、链接、围栏 syntect）；源码按窗格宽度折行、背景纯白；真加粗（雅黑 Bold 与等宽粗体）。
...
- 文本编码自动识别（UTF-8 BOM/UTF-16/GBK）并按原编码回写；外部修改监视自动重载（有未保存修改则询问）；标签右键「移到新窗口」/拖出拆窗、拖入合并（对齐 docview）。
