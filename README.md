...
- Left sidebar (F4): Explorer folder tree (lazy load; single-click selects the row, no text drag; double-click opens a file; darker folder names; toolbar up/back/forward/refresh and an editable absolute path; double-click a folder to expand/collapse; context menu “Set as workspace”) + Outline; filter, scroll highlight, click to jump; width and on/off in settings.json
