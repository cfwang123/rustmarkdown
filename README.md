...
- Source and preview: double-click expands to the characters before whitespace, `' " ( )`, backtick, comma, and Chinese punctuation `，。、‘“’”（）？` (`: / \` stay in the token); triple-click selects the line; leading/trailing whitespace is trimmed from the selection (clicking whitespace selects nothing). Source double-click keeps the selection (no flash). Preview double-click selects the whole paragraph / list item (highlight excludes trailing line gaps); copy does not insert extra spaces between CJK and Latin/digits, and trims ends. Source double-click no longer waits 0.5–1 s, including when scrolled down a long file
...
- Markdown source highlight (heading text near-black, `#` markers by level, gray markers, task boxes `[ ]` light-gray fill / `[x]` light-green fill, inline code gray fill like preview, fenced syntect); monospace prefers Consolas (GVIM-like); headings and `**bold**` use Consolas Bold; line height is font size × 1.45; fenced blocks get a full-width gray fill; drag-select reuses layout (find/preview-map tints are overlays, not a relayout); selection mesh is detached from the paragraph cache so unselected lines keep their colors; selection paints a blue fill without recoloring glyphs (avoids cache tint at the start of a range); long documents drop off-screen meshes every frame (keep glyphs, unique Arc) so drag / Ctrl+A does not clone thousands of row meshes; typing recolors only dirty lines (fences from their start) and reuses paragraph Galleys when wrap jitters within 64 px; Ctrl+click a source `[text](#anchor)` or `[text](file.md)` to jump (same as preview links); bare `http(s)://` URLs and file paths (drive / UNC / relative with extension) are also underlined links and Ctrl+click jumps; Ctrl+click `![alt](image)` opens the image overlay; AccessKit is off so Windows UIA does not freeze a 2000-line source view about 3–10 s after open
