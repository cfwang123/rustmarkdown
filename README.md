...
- **PDF read-only** (aligned with docview continuous pages): pdfium raster per page, stacked vertically; only visible pages are laid out (SumatraPDF-style cache: keep old bitmap while zooming, drop far pages, coalesce stale renders); open at 100%; Ctrl+wheel / Ctrl++- / Ctrl+0 zoom; PgUp/PgDn page, arrows scroll; text select is Sumatra-style yellow highlight; Ctrl+C / right-click copy selected text; outline is a page list; double-click zoom, right-click copy image
