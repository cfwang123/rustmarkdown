...
- Source and preview: double-click expands to the characters before whitespace, `' " ( )`, backtick, comma, and Chinese punctuation `，。、‘“’”（）？` (`: / \` stay in the token); triple-click selects the line (visual wrapped row in preview); leading/trailing whitespace is trimmed from the selection (clicking whitespace selects nothing). Source double-click keeps the selection (no flash). Copy does not insert extra spaces between CJK and Latin/digits. Source double-click no longer waits 0.5–1 s, including when scrolled down a long file
